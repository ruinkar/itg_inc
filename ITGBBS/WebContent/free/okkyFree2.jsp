...
<%!
	...
	//선언문 
		...
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-DD HH:mm");	//한국식 날짜양식 설정

%>

<% 
		//현재페이지 설정
		String pageNum = request.getParameter("pageNum"); //클라이언트가 클릭시 자기자신 호출하면 값 넘어옴
		
		if(pageNum==null)	pageNum="1";	//처음 페이지를 열었을 때
...
<div class="sidebar sidebar-category">
    ...
    <div class="nav-user nav-sidebar">
            ...
            <ul class="nav nav-sidebar">
                <li data-toggle="tooltip" data-container="body" title="로그인"><a href="/login/auth?redirectUrl=%2Farticles%2Fcommunity" class="link"><i class="fa fa-sign-in"></i> <span class="nav-sidebar-label">로그인</span></a></li>
...
<ul class="nav">
    <li><a href="/articles/community" class="link"><span class="nav-sidebar-label nav-sidebar-category-label">All</span> <span class="nav-indicator nav-selected"><span class="nav-selected-dot"></span></span></a></li>
        ...
        <li><a href="/articles/life" class="link"><span class="nav-sidebar-label nav-sidebar-category-label">사는얘기</span> <span class="nav-indicator "><span class="nav-selected-dot"></span></span></a></li>
...
<%	if(count == 0){ //게시판에 출력할 글이 없다면%>
                        <li class="list-group-item  list-group-no-note clearfix">
                            ...
                            <div class="list-group-item-author clearfix">
                                <div class='avatar avatar-list clearfix '><a href='/user/info/3493' class='avatar-photo'><img src='//www.gravatar.com/avatar/904f71d814b2fb4a43c93284d5c8dea9?d=identicon&s=30'/></a> <div class="avatar-info"><a class="nickname" href="/user/info/3493">msb0</a> <div class="activity"><span class="fa fa-flash"></span> 293</div><div class="date-created timeago" title="2016-06-27 11:54:27.0">2016-06-27 11:54:27.0</div></div></div>
                            </div>
                        </li>
<%	}else{ //출력할 글이 있다면%>  
	<%for(int i=0; i<articleList.size(); i++){ 
		BoardDTO article = (BoardDTO)articleList.get(i);%>                      
                        <li class="list-group-item  list-group-no-note clearfix">
                            ...
                            <div class="list-title-wrapper clearfix">
                                <div class="list-tag clearfix">
                                    <span class="list-group-item-text article-id"><%=number-- %></span>
                                    <a href="/articles/life" class="list-group-item-text item-tag label label-info"><i class="fa fa-comments"></i>자유게시판(사는얘기)</a>
                            ...
                            <div class="list-summary-wrapper clearfix">
                                    ...
                                    <div class="list-group-item-summary clearfix">
                                        <ul>
                                            <li class="item-icon-disabled"><i class="item-icon fa fa-comment "></i> 0</li>
                                            <li class="item-icon-disabled"><i class="item-icon fa fa-thumbs-up"></i> 0</li>
                                            <li class=""><i class="item-icon fa fa-eye"></i><%=article.getReadcount() %></li>
                                        </ul>
                                    </div>
                            ...
                            <div class="list-group-item-author clearfix">
                                <div class='avatar avatar-list clearfix '><a href='/user/info/3493' class='avatar-photo'><img src='//www.gravatar.com/avatar/904f71d814b2fb4a43c93284d5c8dea9?d=identicon&s=30'/></a> <div class="avatar-info"><a class="nickname" href="/user/info/3493"><%=article.getWriter() %></a> <div class="activity"><span class="fa fa-flash"></span> 293</div><div class="date-created timeago" title="2016-06-27 11:54:27.0"><%=sdf.format(article.getAdate())%></div></div></div>
                            </div>
                        </li>
	<%	} %>                        
<%	} %>                        
...
<% //페이징 처리: 이번블럭, 현재블럭, 다음블럭   
	if(count > 0){
		...
		for(int i = startPage; i <= endPage; i++){ %>                  
  		...
  		<%}
		//다음블럭  ...23일 경우
		if(endPage < pageCount){  //&& endPage > blockSize %>
            <li><a href="list.jsp?pageNum=<%=startPage+blockSize%>" class="step"><span>...</span></li>
        <%} %>        
<%	}%>                    
                    <li><a href="/articles/community?offset=72020&amp;max=20&amp;sort=id&amp;order=desc" class="step">3602</a></li><li class="next"><a href="/articles/community?offset=20&amp;max=20&amp;sort=id&amp;order=desc" class="step">&raquo;</a></li></ul>
